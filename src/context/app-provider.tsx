...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const refreshAllData = useCallback(async () => {
    ...
    if (hasPermission('COMMANDE_READ')) {
        ...
        fetchCommandesPromise = Promise.allSettled(promises).then(results => {
        ...
        }).catch(err => handleFetchError(err, 'Commandes utilisateur'));
    }
  ...
  const corrigerStock = useCallback(async (produitId: number, lieuStockNom: string, nouvelleQuantite: number) => {
    try {
      ...
      handleGenericError(error, "Erreur de correction du stock");
    }
  }, [refreshAllData, toast, handleGenericError]);
